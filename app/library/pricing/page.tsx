...
export default function PricingPage() {
    const pricingComponents = registry.filter(c => c.category === 'Pricing');
            ...
            <main className="relative z-10 max-w-7xl mx-auto px-6 py-20">
                ...
                <header className="mb-16">
                    <div className="glass-card corner-brackets p-6 inline-block mb-6">
                        <h1 className="text-3xl md:text-5xl font-bold tracking-tight">SaaS Conversion</h1>
                    </div>
                    <p className="text-text-secondary text-lg max-w-2xl mb-6">
                        High-trust pricing tables and calculators. Components designed to convert visitors into customers.
                    </p>
                    <div className="flex items-center gap-4">
                        <span className="glass-panel px-6 py-2 text-sm font-mono uppercase tracking-wider">
                            {pricingComponents.length} Components
                        </span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {pricingComponents.map((comp) => (<PromptCard key={comp.id} component={comp} />))}
                </div>
            </main>
